...
export function getDeploymentEnvStatus(): DeploymentEnvRow[] {
  ...
  return [
    ...
    },
    {
      ...
      class: "SECRET",
      notes: "Email fallback when Admin SMTP is disabled",
      status: present("RESEND_API_KEY") ? "configured" : "not_configured",
    },
    {
      key: "AI_SECRETS_ENCRYPTION_KEY",
      label: "Secrets encryption key",
      class: "SECRET",
      notes: "Required for Admin SMTP password storage (≥16 chars)",
      status: present("AI_SECRETS_ENCRYPTION_KEY") ? "configured" : "not_configured",
    },
    {
      key: "SMTP_HOST",
      label: "SMTP host (env fallback)",
      class: "SECRET",
      notes: "Used when Admin SMTP is disabled",
      status: present("SMTP_HOST") ? "configured" : "not_configured",
    },
    {
      key: "SMTP_PORT",
      label: "SMTP port (env fallback)",
      class: "OPTIONAL",
      notes: "Env SMTP port",
      status: present("SMTP_PORT") ? "configured" : "not_configured",
    },
    {
      key: "SMTP_FROM_EMAIL",
      label: "SMTP from email (env fallback)",
      class: "OPTIONAL",
      notes: "Env SMTP sender",
      status: present("SMTP_FROM_EMAIL") ? "configured" : "not_configured",
    },
    {
